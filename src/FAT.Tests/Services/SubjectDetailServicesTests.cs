...
{
    private static SubjectMaterialService CreateService(FatDbContext db, bool asAdmin = true)
...
{
    private static AssessmentScheduleService CreateService(FatDbContext db, bool asAdmin = true)
        => new(db, asAdmin ? TestCurrentUserContext.Admin() : TestCurrentUserContext.Student(1));
