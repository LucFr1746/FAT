...
public sealed class GradeService(FatDbContext db) : IGradeService
{
    ...
    {
        var targetOrder = await db.Semesters.Where(s => s.SemesterId == semesterId).Select(s => (int?)s.DisplayOrder)
            .SingleOrDefaultAsync(cancellationToken) ?? throw new KeyNotFoundException("Semester not found.");
        if (!await db.Students.AnyAsync(s => s.StudentId == studentId, cancellationToken))
            throw new KeyNotFoundException("Student not found.");
        if (!await db.Courses.AnyAsync(c => c.CourseId == courseId && c.IsActive, cancellationToken))
        ...
        var requirements = await db.Prerequisites.AsNoTracking().Where(p => p.CourseId == courseId)
            .Select(p => new { p.RequiredCourseId, p.Type }).ToListAsync(cancellationToken);
        foreach (var requirement in requirements)
        {
            var satisfied = await db.Enrollments.AnyAsync(e => e.StudentId == studentId &&
                e.CourseId == requirement.RequiredCourseId &&
                ((requirement.Type == PrerequisiteType.Prerequisite && e.Status == EnrollmentStatus.Passed && e.Semester!.DisplayOrder < targetOrder) ||
                 (requirement.Type == PrerequisiteType.Corequisite && e.Status != EnrollmentStatus.Withdrawn && e.Semester!.DisplayOrder <= targetOrder)), cancellationToken);
            if (!satisfied) throw new InvalidOperationException("Course prerequisites have not been satisfied.");
        }
