...
/// <summary>
/// Reads the FLM export from the JSON folder (db/data/json).
///
/// Accepts either the folder itself or any .json inside it, because a file
/// picker naturally lands on a file while the caller naturally has the folder.
/// </summary>
public sealed class JsonFlmDataReader : IFlmDataReader
{
    private const string CurriculaFile = "curricula.json";
    private const string SubjectsFile = "subjects.json";
    private const string AssessmentsFile = "assessments.json";
    ...
    private const string CombosFile = "combos.json";

    /// <summary>
    /// The placeholder subjects.json uses for elective subjects that belong to no
    /// single programme. Their real placement lives in combos.json.
    /// </summary>
    private const string ComboPlaceholder = "(combo)";

    public string SourceName => "JSON folder";
    ...
    {
        var folder = ResolveFolder(path)
            ?? throw new DirectoryNotFoundException($"Không tìm thấy thư mục dữ liệu JSON: {path}");
        ...
        var subjectRows = await JsonFile.ReadAsync(subjectsPath, cancellationToken);

        var subjects = new List<FlmSubjectRow>();
        subjects.AddRange(ReadSubjects(subjectRows));

        // Elective subjects sit in subjects.json under the "(combo)" placeholder
        // with no kỳ, so the pass above cannot place them. combos.json holds the
        // real programme and kỳ for each one. Skipping this file would silently
        // drop 48 subjects - which is exactly the gap that made the JSON and
    ...
    {
        var rows = await JsonFile.ReadAsync(Path.Combine(folder, CurriculaFile), cancellationToken);
    ...
    /// <summary>Subjects that subjects.json already places into a programme and kỳ.</summary>
    ...
    /// <summary>
    /// Elective subjects, placed by combos.json.
    ///
    /// combos.json carries the programme, kỳ, credits and GPA flag but no name or
    ...
    {
        var comboRows = await JsonFile.ReadAsync(Path.Combine(folder, CombosFile), cancellationToken);
        ...
        foreach (var row in comboRows)
        {
            ...
            result.Add(new FlmSubjectRow(
                ...
                SubjectName: FlmValueParser.Truncate(
                             ...
                             ?? subjectCode,
                TermNo: termNo.Value,
                // combos.json is the authority on credits here; subjects.json
    ...
    private static async Task<IReadOnlyList<FlmAssessmentRow>> ReadAssessmentsAsync(
        string folder, CancellationToken cancellationToken)
    {
        var rows = await JsonFile.ReadAsync(Path.Combine(folder, AssessmentsFile), cancellationToken);
    ...
    /// <summary>
    /// Merges the two material sources: materials.json holds bibliographic
    /// entries and files.json holds downloadable links. The workbook presents
        ...
        string folder, CancellationToken cancellationToken)
    {
        ...
        foreach (var row in await JsonFile.ReadAsync(Path.Combine(folder, FilesFile), cancellationToken))
        {
            var subjectCode = FlmValueParser.Clean(Get(row, "subjectCode"))?.ToUpperInvariant();
    ...
    private static async Task<IReadOnlyList<FlmScheduleRow>> ReadSchedulesAsync(
        string folder, CancellationToken cancellationToken)
    {
        var rows = await JsonFile.ReadAsync(Path.Combine(folder, SessionsFile), cancellationToken);
